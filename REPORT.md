...
3. 지방광역시 6개 도시 사이에도 가격 상승 속도와 시기에 차이가 있는가?
...
- **라이선스**: 제공기관은 국토교통부이며, 공공데이터포털 API 페이지의 이용허락범위는 "제한 없음"이다. 원자료에는 매수·매도자 성명 등 개인식별정보가 없고 개인/법인 구분만 있다
...
### 적용한 시계열 분석 기법과 시각화 설계
- **12개월 이동평균**: 월별 중앙값은 지역·월별 거래 수가 적어 들쭉날쭉(노이즈)하고, 부동산은 1년 주기 변동이 있을 수 있다. 직전 12개월 값을 평균하면 이 두 가지가 완화되어 추세가 보인다. 단점은 변화가 늦게 반영되고 첫 11개월 값이 없다는 점이다(시각화 1·2).
- **변화율**: 서울(300\~1,500만원/㎡)과 지방(100\~500)은 수준이 달라 절대 차이 대신 비율로 비교했다. 국면별 누적 변화율과 구성 보정 지수(시각화 3·4)를 썼고, 전년동월대비(YoY)는 계산해 [`yoy_monthly.csv`](data/processed/yoy_monthly.csv)로 저장했으나 그래프에는 쓰지 않았다.
- **격차 배율**: 서울 가격을 지방광역시 통합 가격으로 나눈 값으로, 격차를 하나의 숫자로 추적한다(시각화 2).
- **트렌드/계절성/노이즈**: 트렌드는 장기적인 상승·하락 방향, 계절성은 매년 같은 달에 반복되는 패턴, 노이즈는 이 둘로 설명되지 않는 나머지 변동이다. 분해 결과는 7번에서 다룬다.
- **시간 축 점검**: 연월 순으로 정렬했고, 8개 시리즈×240개월=1,920개 데이터 포인트에 누락이 없음을 전처리 로그로 확인했다.
- **스케일 점검**: 서울과 지방의 수준 차이가 커 시각화 1에서는 지방 도시 간 차이가 평평해 보인다. 그래서 도시별 비교는 2006년=100으로 맞춘 지수(시각화 4)로 본다.
- **집계 단위**: 월별로 집계했다. 일별은 지역당 거래가 적어 불안정하고, 연 단위는 국면 전환과 12개월 주기를 볼 수 없다. 단 변화율 비교(시각화 3·4)는 구성 보정에 칸당 표본이 필요해 연(12개월 창) 단위를 썼다.

### 시각화 1 — 서울 vs 지방광역시 면적당 가격 추이
![가격 추이](images/01_price_trend.png)

원계열(연한 선)과 12개월 이동평균(굵은 선)을 함께 표시했다. 12개월 이동평균 기준으로 서울은 2016년 12월 578만원/㎡에서 2021년 12월 1,220만원/㎡로 5년간 2배 이상 올랐고, 같은 기간 지방광역시 통합은 294에서 358로 완만하게 올랐다. 서울은 2009년 전후 한 차례 오른 뒤 2014년경까지 횡보하다 2017년 이후 가팔라지는 반면 지방은 꾸준히 오르는 모양이다. 이 그래프는 수준 비교라 거래 구성 변화가 섞여 있으므로 변화 크기는 시각화 3·4로 다시 확인한다.

### 시각화 2 — 서울/지방광역시 가격배율(격차 지표) 추이
![격차 배율](images/02_gap_ratio.png)

서울 가격을 지방광역시 통합 가격으로 나눈 배율의 추이다. 배율이 오르면 격차가 커지는 것이고, 내리면 좁혀지는 것이다. 월별 배율은 2015년 7월 1.82배(최저)까지 내려갔다가 2025년 2월 3.88배(최고)까지 올랐고, 12개월 이동평균으로 보면 2009년 7월 3.32배 → 2015년 7월 1.87배 → 2022년 6월 3.45배로 오르내렸다. 즉 격차는 한 방향으로만 커진 것이 아니라 확대와 축소를 반복했다. 다만 수준 비교라 구성 효과가 섞여 있어 격차의 크기는 시각화 4의 구성 보정 기준으로 재확인한다.
...
왼쪽은 8개 시리즈의 구성 보정 가격지수(2006년=100), 오른쪽은 서울 지수를 지방광역시 통합 지수로 나눈 **상대 격차 지수**(2006년=100)를 시각화 2와 같은 방식의 풀링 중앙값 배율과 비교한 것이다. 이 지수는 "2006년에 비해 격차가 몇 % 벌어졌나"만 보여주며, "서울이 지방의 몇 배"라는 수준 비교는 시각화 1·2(중앙값)에서 한다. 구성 보정 기준 격차 지수는 2009년 115, 2015년 74, 2021년 116, 2025년 119로, 풀링 중앙값 배율 기준(134, 81, 146, 137)과 오르내리는 방향은 같지만 진폭이 훨씬 작다. 즉 격차 확대·축소의 흐름은 유지되고, 시각화 2가 그 크기를 과장한다.
...
- **해석(가설)**: 수도권 집중(인구·일자리·인프라)과 유동성 확대기 서울 중심의 투자수요 쏠림이 원인으로 추정된다. 다만 본 데이터에는 소득·인구이동 등 원인 변수가 포함되지 않아 가설 수준이며, 반례로 2013\~2016년 회복기(지방 +33.0% vs 서울 +23.1%)와 2022\~2023년 금리인상기(지방 +5.5% vs 서울 -1.5%)에는 지방이 서울보다 더 올랐다.
...
| README·REPORT 문서 구성과 문장 작성 | 목차·바로가기와 리포트 문장을 빠르게 정리하기 위해 | GitHub에 올린 뒤 실제 렌더링 화면에서 목차 앵커·이미지 링크가 동작하는지 확인하고, 물결표(~)가 취소선으로 깨지는 문제와 수치·문구 오류를 발견해 수정 |
...
Python 3.10 이상에서 실행한다(개발 시 3.14.7). API 키는 공공데이터포털에서 "국토교통부 아파트매매 실거래자료"를 활용신청해 발급받는다.
